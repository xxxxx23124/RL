...
class Config:
    ...
    p_flicker: float = 0.0  # [如果关闭自动课程] 固定的观测值“闪烁”概率，即某个维度的观测值有多大几率被置为0。
    ...
    timesteps_per_batch: int = 6400  # 每个并行环境(worker)一次收集的数据量（时间步数）。
    ...
    encoder_d_model: int =  256 # Transformer模型内部的主要工作维度 (embedding size)。
    encoder_attention_heads: int = 4  # 多头注意力机制中的头数。
    # 前馈网络 (Feed-Forward Network)
    lrnn_state:int = 32
    lrnn_expand:int = encoder_d_model
    encoder_d_ff: int = encoder_d_model * 3  # 前馈网络(FFN)的中间层维度。
    # 层数 (Number of Layers)
    backbone_layers: int = 3  # 主干网络中的Transformer Encoder层数。
    actor_layers: int = 2  # Actor头中的Transformer Encoder层数。实际上在模型中还有一个actor_head的模块，所有实际大小为actor_layers+1
    critic_layers: int = 1  # Critic头中的Transformer Encoder层数。实际大小与actor_layers同理
...
class DepthwiseLinear(nn.Module):
    """(B, L, C) → (B, L, C·K) 逐通道线性；等价于 groups=C 的 1×1 Conv"""
    def __init__(self, channels: int, k: int, bias=True, device=None):
        super().__init__()
        self.k = k
        # weight: (C·k, 1, 1)，bias: (C·k)
        self.conv = nn.Conv1d(
...
class LRNN_StatefulFFN(nn.Module):
    def __init__(self, d_model: int, d_state: int, d_expand: int):
        ...
        # --- 外部的门控和投影层 (类似Mamba的设计) ---
        self.in_proj = nn.Linear(d_model, 2 * d_expand)
        self.out_proj = nn.Linear(d_expand, d_model)

        # --- 核心状态空间模型 (SSM) 参数的投影层 ---
        # 形状: (d_expand, d_state)
        # 初始化为 U[log(0.5), log(1.0)] -> Re(A) in [-0.693, 0]虚部均匀分布在 [-π, π]
        A_log_re = torch.log(torch.rand(d_expand, d_state) * 0.5 + 0.5)
        A_log_im = (torch.rand(d_expand, d_state) * 2.0 - 1.0) * math.pi  # U[-π, π]
        self.A_log = nn.Parameter(torch.complex(A_log_re, A_log_im))

        # B C 为复数
        self.ssm_params_proj = DepthwiseLinear(
            channels=d_expand,
            k=4 * d_state + 1,  # 每个通道输出这么多 scalar
        )
    ...
    def _init_weights(self, module):
        """
        针对 Depth-wise 1×1 Conv 进行定制初始化
        其它 Linear / Conv 均用默认 Xavier。
        """
        if isinstance(module, nn.Linear):
        ...
        if isinstance(module, DepthwiseLinear):
            k = 4 * self.d_state + 1  # 每通道输出维度
            ...
            # ---------------------------
            # 2. 对 Δ 的那一列做特殊处理
            #    Δ 在通道内部的索引是 k-1 (= 4*d_state)
            # ---------------------------
            with torch.no_grad():
                weight = module.conv.weight  # (C·k, 1, 1)
                bias = module.conv.bias  # (C·k)

                # 先把 layout 变为 (C, k)
                weight = weight[:, 0, 0].view(self.d_expand, k)
                bias = bias.view(self.d_expand, k)

                delta_idx = 4 * self.d_state  # = k-1

                target_delta = 0.10  # 想要的初始 Δ
                delta_bias = math.log(math.exp(target_delta) - 1)

                # bias: 直接置常数
                bias[:, delta_idx] = delta_bias

                # weight: 置一个极小值，几乎不依赖 token 内容
                weight[:, delta_idx].fill_(1e-6)
    ...
    def discretize(self, A_log: torch.Tensor, delta: torch.Tensor):
        ...
        A_bar = torch.exp(z)
        expm1_z = torch.expm1(z)
        # 安全处理A的逆
        eps = 1e-6
        A_safe = torch.where(A.abs() < eps, torch.full_like(A, eps), A)
        B_bar_base = delta.to(A_log.dtype) * expm1_z / A_safe
        return A_bar, B_bar_base

    ''' #这个实现会占用更多显存
    def parallel_scan(
    self,
    a_bar: torch.Tensor,          # (B, L, D, S)  复数
    b_bar: torch.Tensor,          # (B, L, D, S)  复数
    u:      torch.Tensor,         # (B, L, D)     实数
    C:      torch.Tensor,         # (B, L, D, S)  复数
    chunk_size: int = 64,
) -> torch.Tensor:                # 返回 (B, L, D) 实数
    """
    基于“块内并行 + 块间 scan” 的线性递推求解。
    """
    B, L, D, S = a_bar.shape
    device, cdtype = a_bar.device, a_bar.dtype

    b_bar = b_bar * u.to(cdtype)[..., None]  # (B, L, D, S)

    n_chunks = (L + chunk_size - 1) // chunk_size
    pad_len  = n_chunks * chunk_size - L
    if pad_len:
        pad = (0, 0, 0, 0, 0, pad_len)  # 只在 sequence 维补 0
        a_bar  = F.pad(a_bar, pad, value=1.0)   # a=1 对递推无影响
        b_bar  = F.pad(b_bar, pad)              # b=0 对递推无影响
        C      = F.pad(C,     pad)

    a_chunks = a_bar.view(B, n_chunks, chunk_size, D, S)
    b_chunks = b_bar.view(B, n_chunks, chunk_size, D, S)
    C_chunks = C.view    (B, n_chunks, chunk_size, D, S)

    # prefix product P_t
    P = torch.cumprod(a_chunks, dim=2)                    # (B,n,C,D,S)
    invP        = torch.reciprocal(P)
    prefix_sum  = torch.cumsum(b_chunks * invP, dim=2)    # ∑ b/P
    h_chunks_0  = prefix_sum * P                          # 从 0 初始得到的 h

    h_last  = h_chunks_0[:, :, -1]    # (B, n_chunks, D, S)
    prod_a  = P[:, :, -1]             # (B, n_chunks, D, S)

    # exclusive prefix product of prod_a
    prefix_prod = torch.cumprod(prod_a, dim=1)
    prefix_prod = torch.roll(prefix_prod, shifts=1, dims=1)
    prefix_prod[:, 0].fill_(1 + 0j)

    # compute exclusive prefix “running h”
    running_h = torch.cumsum(h_last / prefix_prod, dim=1) * prefix_prod
    running_h = torch.roll(running_h, shifts=1, dims=1)
    running_h[:, 0].zero_()          # 第 0 块起始状态设 0

    h_chunks = h_chunks_0 + running_h.unsqueeze(2) * P    # (B,n,C,D,S)

    y_chunks = torch.real((h_chunks * C_chunks).sum(-1))  # (B,n,C,D)
    y = y_chunks.reshape(B, n_chunks * chunk_size, D)[:, :L]

    return y
    '''

    def parallel_scan(self, a_bar, b_bar, u, C, chunk_size: int = 64):
        """
        a_bar, b_bar: (B, L, D, S)   复数
        u           : (B, L, D)      实数
        C           : (B, L, D, S)   复数
        返回        : (B, L, D)      实数
        """
        ...
        # ———————————— 1. 预处理 ————————————#
        b_bar = b_bar * u.to(dtype)[..., None]  # (B,L,D,S)

        # ———————————— 2. 按 chunk 拆分 ————————————#
        n_chunks = (L + chunk_size - 1) // chunk_size
        a_chunks = a_bar.split(chunk_size, dim=1)  # 长度 = n_chunks
        b_chunks = b_bar.split(chunk_size, dim=1)
        C_chunks = C.split(chunk_size, dim=1)

        # ———————————— 3. 每块内部递推 & 收集末状态 ————————————#
        h_chunks, last_states, prod_a = [], [], []
        for a_c, b_c in zip(a_chunks, b_chunks):
            # a_c, b_c : (B, Lc, D, S)
            # 直接做串行递推；Lc ≤ 64 时 GPU 也很快
            h_c = torch.empty_like(b_c)
            h_prev = torch.zeros_like(b_c[:, 0])
            for t in range(a_c.size(1)):
                h_prev = a_c[:, t] * h_prev + b_c[:, t]
                h_c[:, t] = h_prev
            h_chunks.append(h_c)  # (B,Lc,D,S)
            last_states.append(h_prev)  # (B,D,S)
            prod_a.append(torch.prod(a_c, dim=1))  # (B,D,S)

        # ———————————— 4. 块间扫描 ————————————#
        prod_a = torch.stack(prod_a, dim=1)  # (B, n_chunks, D, S)
        last_s = torch.stack(last_states, dim=1)  # (B, n_chunks, D, S)

        # exclusive 前缀乘积
        prefix_a = torch.cumprod(
            torch.cat([torch.ones_like(prod_a[:, :1]), prod_a[:, :-1]], dim=1),
            dim=1)  # (B,n_chunks,D,S)

        # 每块应加的初始状态
        init_states = torch.cumsum(last_s / prefix_a, dim=1) * prefix_a
        init_states = torch.cat(
            [torch.zeros_like(init_states[:, :1]), init_states[:, :-1]], dim=1
        )  # (B,n_chunks,D,S)

        # 把 init_states 也拆块
        init_state_chunks = init_states.unbind(dim=1)  # list, len=n_chunks

        # ———————————— 5. 写回状态并计算输出 ————————————#
        y_chunks = []
        for h_c, a_c, b_c, C_c, s0 in zip(
                h_chunks, a_chunks, b_chunks, C_chunks, init_state_chunks):

            # 重新用 s0 做一次递推（块长很短，成本低）
            state = s0  # (B,D,S)
            y_c = []
            for t in range(a_c.size(1)):
                state = a_c[:, t] * state + b_c[:, t]
                y_step = (state * C_c[:, t]).real.sum(-1)  # (B,D)
                y_c.append(y_step)
            y_chunks.append(torch.stack(y_c, dim=1))  # (B,Lc,D)

        y = torch.cat(y_chunks, dim=1)  # (B,L,D)
        return y
    ...
    def forward(self, x: torch.Tensor, use_h_prev:bool = False):
        ...
        # 动态生成 SSM 复数 B, C 和 实数delta，这意味着对于每个输入的token，我们现在有不同的B和C矩阵，从而解决了内容感知的问题
        ssm_params = self.ssm_params_proj(x_ssm)
        ssm_params = ssm_params.view(
            batch_size, seq_len, self.d_expand, 4 * self.d_state + 1
        )
        ...
        B = torch.complex(B_re, B_im) # # (B, L, d_expand, d_state)
        C = torch.complex(C_re, C_im) # # (B, L, d_expand, d_state)

        delta = F.softplus(delta)  # (B, L, d_expand, 1)

        # --- 离散化 A 和 B ---
        # A_log: (d_expand, d_state), delta: (B, L, d_expand)
        A_bar, B_bar_base = self.discretize(self.A_log, delta)
        # A_bar: (B, L, d_expand, d_state)
        # B_bar: (B, L, d_expand, d_state)

        B_bar = B_bar_base * B  # (B, L, d_expand, d_state)
        ...
        if use_h_prev:
            ...
            y = torch.einsum('bds,bds->bd', h, C[:, 0])
            y = torch.real(y).unsqueeze(1)
        else:  # 训练模式，使用并行扫描
            self.h_prev = None
            y = checkpoint(lambda a, b, u, c: self.parallel_scan(a, b, u, c),
                           A_bar, B_bar, x_ssm, C,
...
class EncoderLayer(nn.Module):
    def __init__(self, args: Config):
        ...
        self.ffd = FeedForward(args.encoder_d_model, args.encoder_d_ff)
        self.lrnn_ffd = LRNN_StatefulFFN(args.encoder_d_model, args.lrnn_state, args.lrnn_expand)
