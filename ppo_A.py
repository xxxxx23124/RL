...
class Transformer(nn.Module):
    def __init__(self, in_dim, action_dim, text_max_len):
        super(Transformer, self).__init__()
        self.args = TransformerConfig(in_dim=in_dim, action_dim=action_dim, text_max_len=text_max_len)
...
class Environment:
    def __init__(self, id, gid, render_mode = None, device="cpu", timesteps_per_batch=8000, text_max_len = 10000, max_timesteps_per_episode=2000):
        ...
        self.actor_critic = Transformer(self.obs_dim, self.act_dim, text_max_len).to(self.device)
...
class PPO:
    def __init__(self, gid="LunarLanderContinuous-v2", render_mode = None):
        ...
        self.actor_critic_optim = torch.optim.NAdam(self.actor_critic.parameters(),lr=1e-4)
        ...
        self.sub_envs = []
        self.max_cpu_threads = 1
        for i in range(self.max_cpu_threads):
            self.sub_envs.append(Environment(i + 1, gid=self.gid, device="cpu", timesteps_per_batch=4800, text_max_len=self.max_timesteps_per_episode*2 + self.timesteps_per_mini_batch, max_timesteps_per_episode = self.max_timesteps_per_episode))
        self.max_gpu_threads = 1
        for i in range(self.max_gpu_threads):
            self.sub_envs.append(Environment(i + 1, gid=self.gid, device="cuda", timesteps_per_batch=4800, text_max_len=self.max_timesteps_per_episode*2 + self.timesteps_per_mini_batch, max_timesteps_per_episode = self.max_timesteps_per_episode))
        for sub_env in self.sub_envs:
            sub_env._set_agent(self.actor_critic)
        #self.sub_envs.append(Environment(0, gid=self.gid, render_mode="human", device="cuda", timesteps_per_batch=4800, text_max_len=self.max_timesteps_per_episode*2 + self.timesteps_per_mini_batch, max_timesteps_per_episode = self.max_timesteps_per_episode))
        self.i = 0
