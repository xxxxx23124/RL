...
class Config:
    ...
    p_flicker = 0.1
    use_automated_curriculum: bool = True
    # 课程难度控制
    initial_p_flicker_mean: float = 0.00  # p_flicker均值的起始点
    target_p_flicker_mean: float = 0.95  # p_flicker均值的目标上限
    # 难度提升的条件
    curriculum_reward_threshold: float = -200.0  # 当最近N轮平均奖励高于此值时，提升难度
    curriculum_eval_episodes: int = 300  # 用于评估是否提升难度的连续轨迹数量
    # 难度提升的步长
    p_flicker_mean_increment: float = 0.05  # 每次提升p_flicker均值的幅度
    # Beta分布的浓度参数 (concentration)
    # 值越大，分布越集中在均值附近；值越小，分布越分散
    ...
    timesteps_per_mini_batch = 1 #代表只处理一个轨迹，因为1太小了
...
class PPO(Environment):
    ...
    def _compute_loss(self):
        ...
        while len(self.all_batch_lens):
            ...
            if self.args.use_gae and not self.args.use_vtrace:
                A_k = self._calculate_gae()
                mini_batch_rtgs = A_k + V.detach()
            elif self.args.use_vtrace:
