...
class Environment:
    def __init__(self, id, gid, render_mode = None, device="cpu", timesteps_per_batch=8000, max_timesteps_per_episode = 2000):
        ...
        self.actor_critic = Transformer(self.obs_dim, self.act_dim, self.max_timesteps_per_episode).to(self.device)
...
class PPO:
    def __init__(self, gid="LunarLanderContinuous-v2", render_mode = None):
        ...
        self.actor_critic = Transformer(obs_dim, act_dim, self.max_timesteps_per_episode).to(self.device)
        ...
        for i in range(self.max_cpu_threads):
            self.sub_envs.append(Environment(i + 1, gid=self.gid,device="cpu", timesteps_per_batch=4800, max_timesteps_per_episode=self.max_timesteps_per_episode))
        self.max_gpu_threads = 1
        for i in range(self.max_gpu_threads):
            self.sub_envs.append(Environment(i + 1, gid=self.gid, device="cuda", timesteps_per_batch=4800, max_timesteps_per_episode=self.max_timesteps_per_episode))
