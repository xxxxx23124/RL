...
class MultiHeadAttention(nn.Module):
    def __init__(self, d_model, num_heads, kernel_size):
        ...
        self.causal_conv_q = nn.Conv1d(d_model, d_model, bias=True,
                                       kernel_size=kernel_size,
                                       groups=d_model,
                                       padding=kernel_size - 1
                                       )
        self.causal_conv_k = nn.Conv1d(d_model, d_model, bias=True,
                                       kernel_size=kernel_size,
                                       groups=d_model,
                                       padding=kernel_size - 1
                                       )
        self.causal_conv_v = nn.Conv1d(d_model, d_model, bias=True,
                                       kernel_size=kernel_size,
                                       groups=d_model,
                                       padding=kernel_size - 1
                                       )
    ...
    def causal_convolution(self, query, key, value):
        seq_len = query.shape[1]
        # [B, C, L]
        query = query.transpose(-1, -2)
        query = self.causal_conv_q(query)[..., :seq_len]
        key = key.transpose(-1, -2)
        key = self.causal_conv_k(key)[..., :seq_len]
        value = value.transpose(-1, -2)
        value = self.causal_conv_v(value)[..., :seq_len]
    ...
    def forward(self, query, key, value, mask, rotary_emb: RotaryEmbedding):
        ...
        query, key, value = self.causal_convolution(self.proj_q(query), self.proj_k(key), self.proj_v(value))
        # (bs, n_heads, seq_len, d_k)
        x = self.attention(query.view(batch_size, seq_len, self.num_heads, self.d_k).transpose(1,2),
                           key.view(batch_size, seq_len, self.num_heads, self.d_k).transpose(1, 2),
                           value.view(batch_size, seq_len, self.num_heads, self.d_k).transpose(1, 2),
                           mask, rotary_emb, seq_len)
...
class LRNN_StatefulFFN(nn.Module):
    """
    一个基于线性RNN理论加强的、带有状态的FFN（循环模式）。
    此版本整合了数值稳定性、优雅实现和效率优化的改进。
    """

    def __init__(self, d_model: int, d_state: int, d_expand: int):
        """
        :param d_model: 输入和输出的维度
        :param d_state: 隐藏状态 h 的维度
        :param d_expand: FFN的中间扩展维度
        """
        ...
        # A: 状态转移矩阵。参数化为负数以保证稳定性。
        # A_log 的范围是 [0, log(d_state)]，所以 exp(A_log) 在 [1, d_state]
        # 最终 A = -exp(A_log) 在 [-d_state, -1]
        A_init = torch.log(torch.rand(self.d_state) * (self.d_state - 1) + 1)
        self.A_log = nn.Parameter(A_init)

        # B: 输入到状态的映射矩阵
        self.B_proj = nn.Linear(self.d_expand, self.d_state, bias=False)
        # B 的参数现在在B_proj里，不再需要独立的B参数

        # C: 状态到输出的映射矩阵
        self.C = nn.Parameter(torch.randn(self.d_expand, self.d_state, dtype=torch.cfloat))

        # Δ (Delta): 可学习的时间步长参数
        self.delta_proj = nn.Linear(d_model, 1)

        # --- 2. 外部的门控和投影层 (类似Mamba的设计) ---
        self.in_proj = nn.Linear(d_model, d_expand * 2)
        self.out_proj = nn.Linear(d_expand, d_model)

    @property
    def A(self):
        """通过 property 确保 A 总是返回负值"""
        return -torch.exp(self.A_log)

    def discretize(self, delta: torch.Tensor):
        """
        使用零阶保持(ZOH)离散化连续时间参数 A 和 B。
        此版本在数值上更稳定。
        delta shape: (B, L, 1)
        """
        # A_bar
        dA = delta * self.A.view(1, 1, -1)  # (B, L, d_state)
        A_bar = torch.exp(dA)

        # B_bar
        # 当dA很小时，(e^dA - 1)/A -> delta
        # 使用阈值切换公式以保证数值稳定
        small_dA_mask = torch.abs(dA) < 1e-6
        B_bar_large = (A_bar - 1) / self.A.view(1, 1, -1)
        B_bar_small = delta.expand_as(B_bar_large)
        B_bar = torch.where(small_dA_mask, B_bar_small, B_bar_large)  # (B, L, d_state)
    ...
    def forward(self, x: torch.Tensor):
        ...
        x_proj, gate = x_and_gate.chunk(2, dim=-1)  # x_proj:(B,L,d_expand), gate:(B,L,d_expand)
        gate = F.silu(gate)

        # --- 核心 SSM 循环 ---

        # 1. (循环外) 一次性计算所有时间步的 delta, A_bar, 和 B_bar
        delta = F.softplus(self.delta_proj(x))  # (B, L, 1)
        A_bar, B_bar = self.discretize(delta)  # A_bar, B_bar 形状均为 (B, L, d_state)

        # 2. (循环外) 一次性将 x_proj 投影到状态维度
        x_b = self.B_proj(x_proj)  # (B, L, d_state)
        ...
        for t in range(seq_len):
            # 状态更新方程: h_t = Ā_t * h_{t-1} + B̄_t * x_b_t
            # 从预先计算好的张量中切片
            h = A_bar[:, t] * h + B_bar[:, t] * x_b[:, t]

            # 输出方程: y_t = C * h_t
            # 使用 torch.real 获取实部作为输出
            y_t = torch.real(F.linear(h, self.C))
            y_outputs.append(y_t)

        y = torch.stack(y_outputs, dim=1)  # (B, L, d_expand)

        # --- 最终输出 ---
        # 将SSM的输出与门控相乘，并投影回d_model
...
class EncoderLayer(nn.Module):
    def __init__(self, args: Config):
        ...
        self.self_attn = MultiHeadAttention(args.encoder_d_model, args.encoder_attention_heads, args.conv_kernel)
        self.lrnn_ffd = LRNN_StatefulFFN(args.encoder_d_model, args.encoder_d_ff, args.encoder_d_ff)
