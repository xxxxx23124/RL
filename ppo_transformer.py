...
class Config:
    ...
    timesteps_per_batch: int = 300  # 每个并行环境(worker)一次收集的数据量（时间步数）。
    ...
    lrnn_state:int = 16
    lrnn_expand:int = encoder_d_model * 2
...
class LRNN_StatefulFFN(nn.Module):
    def __init__(self, d_model: int, d_state: int, d_expand: int):
        ...
        # B C 为复数
        self.ssm_params_proj = nn.Linear(
            d_expand, d_expand * (4 * d_state + 1)
        )
    ...
    def _init_weights(self, module):
        ...
        if module is self.ssm_params_proj:
            with torch.no_grad():
                # 将 [out_features] 重新 view 为 (d_expand , 4*d_state+1)
                bias  = module.bias.view(self.d_expand,  4 * self.d_state + 1)
                weight = module.weight.view(self.d_expand, 4 * self.d_state + 1, self.d_expand)

                target_delta = 0.1
                delta_bias_init = math.log(math.exp(target_delta) - 1)

                # 第 4*d_state 位置正好是 δ
                bias[:, 4 * self.d_state] = delta_bias_init
                weight[:, 4 * self.d_state, :].fill_(1e-6)

    def discretize(self, A_log: torch.Tensor, delta: torch.Tensor):
        """
        将连续时间 SSM 参数离散化 (Zero-Order Hold)。

        输入:
            A_log : (d_expand , d_state)      复数    —— log(-A_continuous)
            delta : (B, L, d_expand, 1)       实数    —— 步长 Δ>0

        输出:
            A_bar        : (B, L, d_expand, d_state)  复数
            B_bar_base   : (B, L, d_expand, d_state)  复数
                           （尚未与动态 B 相乘的基准值）
        公式:
            Ā = exp(Δ · A)
            B̄ = Δ · (Ā − I) · A⁻¹
        """
        A_log64 = A_log.to(torch.complex128)          # (D,S)   complex128
        delta64 = delta.to(torch.float64)             # (B,L,D,1)  float64

        A_c = (-A_log64).exp()                        # (D,S)    complex128
        A_c = A_c.unsqueeze(0).unsqueeze(0)           # (1,1,D,S) broadcast

        dA   = delta64.to(A_c.dtype) * A_c            # (B,L,D,S) complex128
        A_bar = dA.exp()                              # exp(ΔA)

        eps = 1e-8
        B_bar_base = delta64.to(A_c.dtype) * (A_bar - (1.0 + 0j)) / (A_c + eps)

        A_bar       = A_bar.to(A_log.dtype)
        B_bar_base  = B_bar_base.to(A_log.dtype)
        return A_bar, B_bar_base

    def parallel_scan(self, A_bar, B_bar, u, C):
        pass
    ...
    def forward(self, x: torch.Tensor, use_h_prev:bool = False):
        ...
        if use_h_prev:
            ...
            self.h_prev = None
            #y = self.parallel_scan(A_bar, B_bar, x_ssm, C)
