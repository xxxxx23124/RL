...
class LRNN_StatefulFFN(nn.Module):
    ...
    def parallel_scan(self, A_bar, B_bar, u, C):
        ...
        输入:
            A_bar : (B, L, D, S)  复数  —— aₜ
            B_bar : (B, L, D, S)  复数  —— 用来算 bₜ
            u     : (B, L, D)     实数  —— 驱动输入
            C     : (B, L, D, S)  复数
        输出:
            y     : (B, L, D)     实数
        """
        Bsz, L, D, S = A_bar.shape
        dtype = A_bar.dtype

        # 先得到每个时间步自己的 (aₜ , bₜ)
        a = A_bar.clone()                                          # (B, L, D, S)
        b = (B_bar * u.unsqueeze(-1).to(dtype)).clone()            # (B, L, D, S)

        # Blelloch / Hillis-Steele 扫描
        step = 1
        while step < L:
            # 为了避免原地覆盖造成的先写后读冲突，先 copy 一份
            a_curr = a[:, step:].clone()        # “右半边”  长度 L-step
            b_curr = b[:, step:].clone()
            a_prev = a[:, :-step].clone()       # “左半边”  长度 L-step
            b_prev = b[:, :-step].clone()

            # 复合:  F_curr ∘ F_prev
            a[:, step:] = a_curr * a_prev                      # a_new
            b[:, step:] = a_curr * b_prev + b_curr             # b_new

            step <<= 1  # ×2

        # 此时 b 即为所有前缀变换在 x₀ = 0 时得到的状态 xₜ
        x = b                                              # (B, L, D, S)
        y = (x * C).real.sum(-1)                           # (B, L, D) 只取实部
        return y
