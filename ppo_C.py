...
class Environment:
    ...
    def _report_info(self):
        print(f"环境id：{self.id}：轨迹总奖励总览：",
              pandas.Series([np.sum(ep_rews) for ep_rews in self.batch_ep_rews]).describe(),
              f"环境id：{self.id}：时间步总览：",
              pandas.Series(self.batch_lens).describe(),
              f"环境id：{self.id}：总用时{np.sum(self.ep_time_consume)}秒，时间消耗总览：",
              pandas.Series(self.ep_time_consume).describe(),
              sep='\n')
        ...
        for j in range(self.n_updates_per_iteration):
            ...
            while len(self.all_batch_lens):
                ...
                if approx_kl > self.target_kl:
                    continue
                print(f"目前为第{self.i}轮的第{mini_batch_id}个mini_batch的第{j}轮训练，该mini_batch的样本数为{np.sum(self.mini_batch_lens)}，approx_kl为{approx_kl}")
